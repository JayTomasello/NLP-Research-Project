...
def process_scores(score_container, setting):
    ...
    if setting == 0:
        score_list_out = [sum(score_dict['fear']), sum(score_dict['anger']), 
                          sum(score_dict['anticipation']), sum(score_dict['trust']), 
                          sum(score_dict['surprise']), sum(score_dict['sadness']), 
                          sum(score_dict['disgust']), sum(score_dict['joy']), 
                          sum(score_dict['positive']), sum(score_dict['negative'])]
...
def nrc_chart_emotions_interval_line(scores_dict, plot_title):
    ...
    mp.plot(score_key_list, score_value_list[1], label='Anger', color='firebrick', marker='o')
    mp.plot(score_key_list, score_value_list[2], label='Anticipation', color='grey', marker='o')
    mp.plot(score_key_list, score_value_list[3], label='Trust', color='paleturquoise', marker='o')
    mp.plot(score_key_list, score_value_list[4], label='Surprise', color='yellow', marker='o')
    mp.plot(score_key_list, score_value_list[5], label='Sadness', color='royalblue', marker='o')
    mp.plot(score_key_list, score_value_list[6], label='Disgust', color='yellowgreen', marker='o')
    mp.plot(score_key_list, score_value_list[7], label='Joy', color='pink', marker='o')
    mp.title(plot_title)
    mp.xticks(rotation = 45)
    current_values = mp.gca().get_yticks()
    mp.gca().set_yticklabels(['{:,.0f}'.format(x) for x in current_values])
    mp.xlabel('Month')
    mp.ylabel('Emotion Score')
    mp.legend()
    mp.show()
